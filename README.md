...
订阅脚本会同时启动 mihomo 的控制接口（只监听 `127.0.0.1`），并把下面两个变量写入 `GITHUB_ENV`：

| 环境变量 | 说明 |
| --- | --- |
| `CHECKIN_MIHOMO_API` | 控制接口地址，如 `http://127.0.0.1:9090` |
| `CHECKIN_MIHOMO_SECRET` | 访问密钥，未设置 `CONTROLLER_SECRET` 时随机生成 |

控制接口把节点暴露成 `CHECKIN` 分组的一个可选列表，可以枚举、查看和切换：

```
GET /proxies/CHECKIN              查看全部分组成员与当前选择
PUT /proxies/CHECKIN {"name": ..} 切换到指定节点
```

分组结构是 `AUTO`（`url-test`，自动选路 + 故障转移）套在 `CHECKIN`（`select`）里，默认选中 `AUTO`——**不做任何切换时行为与以前完全一致**。

`CONTROLLER_PORT` 可覆盖控制接口端口（默认 `9090`），`CONTROLLER_SECRET` 可指定固定密钥。

### 按账号轮转出口节点

配置了 `CHECKIN_MIHOMO_API` 与 `CHECKIN_PROXY_URL` 后，**每次账号尝试之前**会自动切换一个出口节点：

- 优先挑出口 IP 本轮没用过的节点；同一 IP 背后的多个节点（机场常见的「同机多端口」）会被跳过
- 节点池绕完一圈后按「循环复用」处理，并明确打出 `节点池已绕回一圈` 告警——否则「IP 不够」和「节点都好」在日志里长得一模一样
- 节点不可达、切换失败、控制接口挂掉，一律**降级为沿用当前出口**，绝不中断签到
- 只对 `use_proxy: true` 的 provider 生效（当前是 `agentrouter`）；为直连账号轮转不仅无用，还会白占一个出口 IP 名额
- **候选顺序会被打乱**。订阅通常按地区排列、同地区节点常共用出口 IP（实测中东京的两个节点出口相同），线性扫描会把探测预算全烧在同一个 IP 簇里，扫不到后面的新鲜节点
- **排除伪节点**。机场常在订阅里塞「剩余流量：xx GB」这类信息占位，它们实际是直连。脚本会先不加代理查一次 runner 自己的出口 IP，任何出口等于它的「节点」都会被排除
- 单次轮转最多做 **8 次出网探测**（不是最多看 8 个候选）；已探明且出口被占用的节点免费跳过，不占预算

这一步的意义在于：出口 IP 是被 WAF 按请求量限流的，所以**「换 IP 重试」才是有意义的重试**，在同一个 IP 上重试只是浪费次数。

未配置控制接口时整个功能关闭，行为与以前一致。

### 测量真实出口 IP

机场常见做法是同一台机器开多个端口，名义上几十个节点实际只有几个出口 IP。轮转节点前先量一下：

```bash
uv run python scripts/probe_nodes.py
```

输出节点总数、可用节点数、**去重后的出口 IP 数**，以及每个 IP 对应哪些节点。这个数字决定「按账号分散出口」是否可行——IP 数少于账号数时，分散的收益有限。

脚本会逐个切换节点并查询出口 IP，注意每个节点都会真的产生一次出网请求。
